...
# ============================================================================
# PPO Training — full-episode on-policy rollout, GAE-lambda advantages,
...
# ============================================================================
def train_ppo(env, epochs=1000, gamma=0.99, lr=1e-4,
              value_coef=0.5, entropy_coef=0.01, gae_lambda=0.95,
              clip_eps=0.2, ppo_epochs=10, minibatch_size=64):
    ...
    for ep in range(epochs):
        # ---- Collect one full episode under the current (frozen) policy ----
        state, _ = env.reset()
        done = False
        ep_reward = 0.0
        ...
        # ---- GAE-lambda advantages + value targets, computed once per episode.
        # Every episode here is a full pass over the dataframe and always ends
        # with done=True, so there is no bootstrap value beyond the episode end. ----
        with torch.no_grad():
        ...
        # ---- K epochs of clipped-surrogate minibatch SGD over the episode ----
        ...
        if ep % 10 == 0:
            print(f"[Hard-PPO] Ep {ep:04d} | Reward: {ep_reward:.4f}")
...
# Plotting helpers
# ============================================================================
def plot_training_progress(rewards, path='results/hard_ppo_training.png'):
    ...
    plt.title('PPO Training — Hard Regime Routing')
    plt.legend()
    plt.grid(alpha=0.3)
...
def plot_wealth_over_time(env, initial, path='results/hard_ppo_wealth.png'):
    import matplotlib.pyplot as plt
...
# ============================================================================
def run_experiment(seed: int = 0, out_dir: str = "results/hard_ppo",
    ...
    # ── Train ─────────────────────────────────────────────────────────────
    actor, critic, rewards = train_ppo(env_train, epochs=epochs)
