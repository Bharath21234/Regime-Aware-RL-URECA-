"""
Main Entry point for Probabilistic MoE FinRL System — PPO version.
...
# ============================================================================
# Training (PPO Core) — full-episode on-policy rollout, GAE-lambda advantages,
...
def train(env, epochs=1000, gae_lambda=0.95, clip_eps=0.2, ppo_epochs=10, minibatch_size=64):
    ...
    print(f"Starting MoE PPO Training on {DEVICE} for {epochs} epochs...")
    for ep in range(epochs):
        ...
        if ep % 10 == 0:
            print(f"[MoE-PPO] Ep {ep:04d} | Reward: {ep_reward:.4f} | PortVal: ${env.portfolio_value:,.2f}")

    return actor, critic, rewards_history

def plot_training_progress(rewards, save_path='results/finrlmain_ppo_training.png'):
    ...
    plt.title('MoE PPO Training Progress')
    plt.legend()
...
# ============================================================================
def run_experiment(seed: int = 0, out_dir: str = "results/moe_ppo",
...
if __name__ == "__main__":
    ...
    def plot_wealth_over_time(asset_memory, date_memory, initial_amount, save_path='results/wealth_over_time_ppo.png'):
        import matplotlib.pyplot as plt
    ...
    plot_metrics_over_time(
        ...
        save_path="results/moe_ppo_metrics_over_time.png", rolling_window=20,
    )
