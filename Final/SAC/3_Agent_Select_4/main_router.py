"""
Main entry point for the Learned-Router MoE FinRL variant (Select_4) — SAC version.
...
# 3_Agent_Select_1's SAC version for the detailed comparison-to-A2C note. ──
def train(env, epochs=300):
    ...
    trunk = ActorLearnedRouter(obs_dim, act_dim).to(DEVICE)
    return train_sac(env, trunk, epochs=epochs, device=DEVICE, tag="ROUTER-SAC")

def plot_training_progress(rewards, save_path='results/router_sac_training.png'):
    ...
    plt.title('Learned-Router SAC Training Progress')
    plt.legend()
...
def plot_metrics_over_time(portfolio_return_memory, asset_memory, dates,
                           ...
                           save_path='results/router_sac_metrics_over_time.png',
                           rolling_window=20):
    import matplotlib.pyplot as plt
    os.makedirs('results', exist_ok=True)
...
def run_experiment(seed: int = 0, out_dir: str = "results/router_sac",
...
if __name__ == "__main__":
    metrics = run_experiment(seed=0, out_dir="results/router_sac")
