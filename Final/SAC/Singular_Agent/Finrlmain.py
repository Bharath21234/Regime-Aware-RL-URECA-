...
def train(env, epochs=300):
    ...
    return train_sac(env, trunk, epochs=epochs, device=DEVICE, tag="BASELINE-SAC")

def plot_training_progress(rewards, path='results/baseline_sac_training.png'):
    ...
    plt.title('SAC Training — Flat-State Baseline (No Gating)')
    plt.legend()
    plt.grid(alpha=0.3)
...
def plot_wealth_over_time(env, initial, path='results/baseline_sac_wealth.png'):
    import matplotlib.pyplot as plt
