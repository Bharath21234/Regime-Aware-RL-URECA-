...
if os.environ.get("MULTI_ASSET", "0") == "1":
    TICKER_LIST = sorted(TICKER_LIST + ['TLT', 'IEF', 'GLD', 'SHY'])
    print(f"MULTI_ASSET=1: universe extended to {len(TICKER_LIST)} assets "
          "(+TLT/IEF/GLD/SHY)")
...
# ---- BASELINE (no gate): constant regime column in place of the HMM ----
# The env appends x[-1] = future_regime; feeding a constant 0.0 keeps the
# state dimensionality identical to Hard RA-RL (1597) while carrying ZERO
# regime information -- a pure information-removal control.
print("\nBaseline (no gate): constant regime column, no HMM.")
regime_df = pd.DataFrame({"date": sorted(df.date.unique())})
regime_df["future_regime"] = 0
...
# ============================================================================
# Actor — single head, no regime routing (Baseline / no gate)
# ============================================================================
class Actor(nn.Module):
    """
    Baseline actor: identical trunk to Hard RA-RL, ONE output head, no
    routing. x[-1] is the constant placeholder regime column (always 0).
    """
    ...
    for ep in range(epochs):
        ...
        while not done:
            ...
            if len(r_buf) >= batch_size:
                ...
                # Baseline's early-peak-then-degrade pattern observed
...
# ============================================================================
def run_experiment(seed: int = 0, out_dir: str = "results/baseline",
    ...
    plot_metrics_over_time(
        ...
        title_prefix=f"Baseline (Select_1) — Seed {seed}",
...
if __name__ == "__main__":
    ...
    print("\nCOMPREHENSIVE PERFORMANCE METRICS (Baseline)")
    print("-" * 44)
